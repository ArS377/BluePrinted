...
export function buildFallbackDiagnosis(trace) {
  ...
  if (!firstError) {
    return {
      ...
      nextStep: trace.projectId === "sample" ? "In the inspector, turn on Make the next save fail. Save another note to compare the results." : "Compare this trace with another recorded action to look for differences.",
      mode: "local"
    };
  }
  ...
  if (sanitizedTrace.fault === "reject_database_write") {
    return {
      summary: "The test failure rejected this save.",
      cause: "The sample stopped the write before calling storage.",
      ...
      nextStep: "Retry the save. The one-request test has reset, so the next write can reach storage.",
      mode: "local"
    };
  }
